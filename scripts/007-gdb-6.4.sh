...
# gdb-6.4.sh by Dan Peori (danpeori@oopo.net)
 ...
 ## Configure the build.
 CFLAGS="-g -O2 -Wno-error=unused-but-set-variable -Wno-error=implicit-fallthrough= -Wno-error=unused-value -Wno-error=cast-function-type -Wno-error=format-overflow= -Wno-error=pointer-compare -std=gnu89" ../configure --prefix="$PSPDEV" --target="psp" --disable-nls || { exit 1; }
